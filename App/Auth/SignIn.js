...
export const SignIn = ({ navigation }) => {
  ...
  const { signIn, snsSetting } = useContext(AuthContext);
  ...
  const [permissionLocCheck, setPermissionLocCheck] = useState(false);
  const [permissionReadCheck, setPermissionReadCheck] = useState(false);
  const [permissionWriteCheck, setPermissionwriteCheck] = useState(false);
  ...
  const getBLEPermission = () => {
    bleManager.state().then((res) => {
      if (res === 'PoweredOff') {
        ...
        navigation.navigate('FindPicoToScan_SignIn', { from: 'auth', strings: strings });
      }
    });
  };

  const onBleGoScan = () => {
    console.log('in ONBLEGOSCAN');
    bleManager.enable();
    setScanLoading(true);
    setTimeout(() => {
      setBleModal(false);
  ...
  useEffect(() => {
    if (permissionLocCheck === true) {
      requestReadPermissions();
    }
  }, [permissionLocCheck]);

  useEffect(() => {
    if (permissionReadCheck === true) {
      ...
      <View style={styles.container}>
        <View style={styles.picoHomeImage}>
          <Image
            ...
            style={{ height: 130, width: 130, marginTop: height * 0.06 }}
          />
        </View>
        <View style={styles.signInViewStyle}>
          ...
          <TouchableOpacity style={styles.buttonStyle} onPress={() => signInBtn()}>
            <Text style={styles.buttonText}>{strings.login_button_login}</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.buttonStyle2} onPress={() => navigation.push('SignUp1')}>
...
const styles = StyleSheet.create({
  ...
  scanAlertPosition: { position: 'absolute', right: 50, top: 23 },
  modalContainer: {
    width: width * 0.9,
