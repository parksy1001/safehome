...
export default class FindPicoToScan extends Component {
  constructor(props) {
    super(props);
    this.state = {
      scanning: false,
    ...
    return (
      <View style={styles.container}>
        ...
        {this.state.isConnected === -1 ? null : this.state.isConnected === 1 ? (
          this.props.navigation.navigate('Scan', { area })
        ) : (
          // <CustomModal
          ...
          <Modal isVisible>
            <View style={styles.home_modalContainer}>
              <View style={styles.home_modalHeaderTextView}>
                <Text style={styles.home_modalHeaderText}>{strings.connecting_popup_error_title}</Text>
              </View>
              <View style={styles.home_modalSubTextView}>
                <Text style={styles.home_modalSubText2}>{strings.connecting_popup_error_contents_update}</Text>
                <Text style={styles.home_modalSubText}>{strings.connecting_popup_error_contents_update1}</Text>
                <Text style={styles.home_modalSubText}>{strings.connecting_popup_error_contents_update2}</Text>
                <Text style={styles.home_modalSubText}>{strings.connecting_popup_error_contents_update3}</Text>
              </View>
              <TouchableOpacity onPress={() => this.props.navigation.navigate('RealTime')}>
  ...
  handleStopScan() {
    ...
    for (const [key, value] of this.state.peripherals) {
      if (value.name === 'Safe-Home' || value.name === 'Bandi-Pico') {
        picos.push(value);
      }
    }
  ...
  */
  ...
  async readData(device, peripheral) {
    //console.log(device.isConnected+"222222222222222222222222222222");
...
const DeviceInfo = {
  ...
  reload: function () {
    //console.log(device.isConnected+"29999999999999999");
    ...
    if (!DeviceInfo.setup) {
      ...
      bleManager.readCharacteristicForDevice(device.id, serviceUUID, characteristicUUID).then((value) => {
        console.log('start read data in else:');
        let data = getValues(value.value);
        DeviceInfo.data = data;
        return data;
      });
...
function getValues(val) {
  ...
  return {
    ...
    pm10: {
      value: val2[2] * 256 + val2[3],
      //level: val2[5],
    },
    temp: {
      value: parseInt((val2[4] * 256 + val2[5]) / 10),
      //level: val2[8],
    },
    humd: {
    ...
    co2: {
      value: val2[14] * 256 + val2[15],
      //level: val2[14],
    },
    vocs: {
      value: val2[16] * 256 + val2[17],
      //level: val2[17],
    },
    co: {
      value: val2[8] * 256 + val2[9],
      //level: val2[17],
    },
    ch4: {
      value: val2[10] * 256 + val2[11],
      //level: val2[17],
    },
