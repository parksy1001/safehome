...
import { AuthContext, LanguageContext, SettingContext, UserContext, UserDetailInfoContext, SnsContext } from '../../../context';
...
export const Profile = ({ navigation }) => {
  ...
  const passwordValidation = (text, type) => {
    ...
    // const pwEx = /^.*(?=^.{9,}$)(?=.*\d)(?=.*[a-zA-Z]).*$/;
    const pwEx = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{9,}$/;
        ...
        {isLoading ? (
          <View>
            ...
            {/* {sns == 'noSns' ? (
              ...
              <View style={{ height: height * 0.29 }}></View>
            )} */}
